...
namespace MyMentor.API.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(IAccountManager accountManager, IAuthorizationService authorizationService, IUnitOfWork unitOfWork, ApplicationDbContext context)
        {
        ...
        {
            return await GetUserById(HelperMethods.GetUserId(User));
        }

        [HttpGet("user/{username}/login/{password}")]
        [ProducesResponseType(200, Type = typeof(UserInfoDTO))]
        [ProducesResponseType(401)]
        public IActionResult LoginUser(string username, string password)
        {
            var user = _unitOfWork.Users.GetUserWithDetails(username, password);
            if (user != null)
            {
                var dto = new UserInfoDTO(user);
                return Json(dto);
            }
            else
            {
        ...
        {
            return await UpdateUser(HelperMethods.GetUserId(User), user);
        }
        ...
        public async Task<IActionResult> UpdateUser(string id, [FromBody] UserEditViewModel user)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (HelperMethods.GetUserId(User) == id)
                {
        ...
        [ProducesResponseType(204)]
        [ProducesResponseType(400)]
        public async Task<IActionResult> UpdateCurrentUser([FromBody] JsonPatchDocument<UserUpdateViewModel> patch)
        {
            return await UpdateUser(HelperMethods.GetUserId(User), patch);
        }
        ...
        [ProducesResponseType(403)]
        [ProducesResponseType(404)]
        public async Task<IActionResult> UpdateUser(string id, [FromBody] JsonPatchDocument<UserUpdateViewModel> patch)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                UserUpdateViewModel userPVM = Mapper.Map<UserUpdateViewModel>(appUser);
                // patch.ApplyTo(userPVM, ModelState);

                // TODO: Andrew Stolzle Find out why the commented line above is giving an error. Otherwise
                // change it to set it manually
        ...
        {
            var userId = HelperMethods.GetUserId(User);
